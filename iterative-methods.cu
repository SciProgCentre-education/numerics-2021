    ...
    float omega)
{
    ...
    check_cuda(crow_indices, col_indices, values, b);

    auto solver = Jacobi<SparseMatrixView<float, Devices::Cuda>>{};
    solver.setOmega(omega);

    return solve<float, Devices::Cuda>(
    ...
    float omega)
{
    ...
    auto solver = SOR<SparseMatrixView<float, Devices::Cuda>>{};
    solver.setOmega(omega);
